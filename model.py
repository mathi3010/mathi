...
class PageHeader(QWidget):
    ...
    def __init__(self, title_text="VisI CoNe", parent=None):
...
class FeatureResult:
    ...
    measured: float
    pass_: bool
    deviation: float  # signed (measured - target/mean)
    limit_low: float
    limit_high: float
...
class QCResult:
    overall_pass: bool
    mode: str
    details: List[FeatureResult]

def _detect_mode(df: pd.DataFrame) -> str:
    cols = {c.lower() for c in df.columns}
    if {"min", "max"}.issubset(cols):
        return "minmax"
    if {"mean", "tolerancepct"}.issubset(cols):
        return "mean_pct"
    if {"mean", "stddev", "k"}.issubset(cols):
        return "mean_k_sigma"
    raise ValueError("Excel columns not recognized. Use either "
                     "[Feature,Min,Max] OR [Feature,Mean,TolerancePct] "
                     "OR [Feature,Mean,StdDev,K].")

def load_good_model(xlsx_path: str) -> Tuple[pd.DataFrame, str]:
    if not os.path.exists(xlsx_path):
        raise FileNotFoundError(f"Model file not found: {xlsx_path}")
...
def compare_features(
    ...
    """
    live_features: {'thread_diameter': 5.03, 'ring_width': 2.04, ...}
    require_all: if True -> overall PASS only if all features pass.
    """
    # Build lookup for model rows by feature name (case-insensitive)
    model_index = {str(row["Feature"]).strip().lower(): i for i, row in model_df.iterrows()}
    details: List[FeatureResult] = []

    for feat, measured in live_features.items():
        key = feat.strip().lower()
        if key not in model_index:
            # If a live feature isn't in the model, treat as fail (or skip).
            details.append(FeatureResult(
                feature=feat, target_str="N/A", measured=measured,
                pass_=False, deviation=np.nan, limit_low=np.nan, limit_high=np.nan
            ))
            continue

        row = model_df.iloc[model_index[key]]
        if mode == "minmax":
            lo = float(row["Min"])
            hi = float(row["Max"])
            # Use mid-point as "target" text
            target = (lo + hi) / 2.0
            pass_ = (lo <= measured <= hi)
            details.append(FeatureResult(
                feature=feat,
                target_str=f"[{lo:.4f} .. {hi:.4f}]",
                measured=measured,
                pass_=pass_,
            ...
            mean = float(row["Mean"])
            tol_pct = float(row["Tolerancepct"])
            lo = mean * (1 - tol_pct/100.0)
            hi = mean * (1 + tol_pct/100.0)
            pass_ = (lo <= measured <= hi)
            details.append(FeatureResult(
                feature=feat,
                target_str=f"{mean:.4f} ± {tol_pct:.2f}%",
                measured=measured,
                pass_=pass_,
                deviation=measured - mean,
                limit_low=lo,
                limit_high=hi
            ))

        elif mode == "mean_k_sigma":
            mean = float(row["Mean"])
            ...
            details.append(FeatureResult(
                feature=feat,
                target_str=f"{mean:.4f} ± {k:.2f}σ (σ={std:.4f})",
                measured=measured,
                pass_=pass_,
                deviation=measured - mean,
                limit_low=lo,
    ...
    if require_all:
        overall = all(d.pass_ for d in details if not np.isnan(d.deviation))
    else:
        # Majority pass criterion
        passes = [d.pass_ for d in details if not np.isnan(d.deviation)]
        overall = (sum(passes) >= max(1, int(0.6 * len(passes))))  # e.g., 60% rule

    return QCResult(overall_pass=overall, mode=mode, details=details)

# --- Optional helpers for your Train page I/O ---

def save_features_to_excel(out_path: str, features: Dict[str, float]):
    """
    Save a single sample's measured features to an Excel row.
    Useful for logging/training data. Appends if file exists.
    """
    df_row = pd.DataFrame([{"Feature": k, "Value": v} for k, v in features.items()])
    if os.path.exists(out_path):
        # Append as a new sheet or merge; here we append a sheet with an index
        with pd.ExcelWriter(out_path, mode="a", if_sheet_exists="overlay", engine="openpyxl") as w:
            df_row.to_excel(w, sheet_name="captures", index=False, header=w.sheets.get("captures") is None)
    else:
        with pd.ExcelWriter(out_path, engine="openpyxl") as w:
            df_row.to_excel(w, sheet_name="captures", index=False)



#========================================================================================
# --- imports (ensure these exist once in your project) ---
import time
from pathlib import Path
import cv2
import numpy as np
...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QFrame, QPushButton, QDialog

# from your_project import PageHeader, ref_dirs, CameraGuard, process_image_to_dfs_only, compare_df, verdict, db_insert_prediction, PredictSetupDialog
# ^ keep your own imports as they already are

class PredictionPage(QWidget):
    """
    VisI Cone – Prediction (with similarity gate & strict empty-data handling)
    ...
    def __init__(self, parent=None):
        ...
        self._ref_image_bgr = None        # <-- store the actual trained reference image
        self._ref_name = None
        self._job_info = None

        # similarity gate thresholds (tune as needed)
        self._sim_thresh_hist = 0.60   # HSV histogram correlation (0..1)
        self._sim_thresh_kp   = 18     # ORB good match count
        ...
        self._frame_counter = 0   # interval trigger for AUTO
        ...
        main.addWidget(PageHeader("VisI CoNe"))
    ...
    def _build_reference_from_latest(self, df_all, chosen_name: str) -> bool:
        ...
        # keep a copy of the actual trained reference image (for similarity gate)
        self._ref_image_bgr = img.copy()
        ...
        # small user feedback
        self.card.setText(f"Reference set → {self._ref_name} (latest). AUTO ready.")
        return True
    ...
    def _upd(self):
        ...
        # AUTO: interval-based trigger (every ~10 frames)
    ...
    # ---------- similarity gate ----------
    def _quick_similarity_gate(self, bgr_frame):
        """
        Fast coarse check: is the current frame sufficiently similar
        to the trained reference? Uses HSV histogram correlation + ORB keypoints.
        Returns (ok: bool, score_hist: float, good_matches: int)
        """
        if self._ref_image_bgr is None:
            return True, 1.0, 999  # no gate if ref image unavailable

        try:
            # --- Histogram (HSV) ---
            ref_hsv = cv2.cvtColor(self._ref_image_bgr, cv2.COLOR_BGR2HSV)
            cur_hsv = cv2.cvtColor(bgr_frame,           cv2.COLOR_BGR2HSV)

            def center_roi(img, frac=0.6):
                h, w = img.shape[:2]
            ...
            ref_roi = center_roi(ref_hsv, 0.6)
            cur_roi = center_roi(cur_hsv, 0.6)

            hist_ref = cv2.calcHist([ref_roi],[0,1],None,[50,60],[0,180,0,256])
            hist_cur = cv2.calcHist([cur_roi],[0,1],None,[50,60],[0,180,0,256])
            ...
            ref_gray = cv2.cvtColor(self._ref_image_bgr, cv2.COLOR_BGR2GRAY)
            cur_gray = cv2.cvtColor(bgr_frame,           cv2.COLOR_BGR2GRAY)
            ...
            # If something fails, do not block by similarity gate
            return True, 1.0, 999

    # ---------- prediction ----------
    def _predict_on(self, bgr_frame):
        # 0) Similarity gate: block totally different views/objects
        ok_sim, corr, good_kp = self._quick_similarity_gate(bgr_frame)
        if not ok_sim:
            self._last_color = "#DC2626"
            self.cam.setStyleSheet("border: 3px solid #DC2626; border-radius:12px; background:#000;")
            self.card.setText(
                f"BAD ❌\nDifferent from trained reference\n"
                f"hist_corr={corr:.2f}, good_kp={good_kp} "
                f"(needs ≥ {self._sim_thresh_hist:.2f}, {self._sim_thresh_kp})"
            )
            # Log the failure to Excel for traceability
            self._append_result_to_excel({
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                "job": (self._job_info or {}).get("job", ""),
                "operator_id": (self._job_info or {}).get("operator_id", ""),
                "notes": "Similarity gate failed",
                "ref_name": (self._ref_name or "unknown"),
                "is_good": False,
                "thread_bad_pct": 100.0,
                "ring_bad_pct": 100.0,
                "verdict": "Bad (similarity gate)",
            })
            return

        # 1) Process test frame
        df_tst_thread, df_tst_ring = process_image_to_dfs_only(bgr_frame)

        # 2) Compare against references (only if both sides exist)
        cmp_thread = compare_df(self.df_ref_thread, df_tst_thread) if not self.df_ref_thread.empty and df_tst_thread is not None and not df_tst_thread.empty else pd.DataFrame()
        cmp_ring   = compare_df(self.df_ref_ring,   df_tst_ring)   if not self.df_ref_ring.empty   and df_tst_ring   is not None and not df_tst_ring.empty   else pd.DataFrame()

        # 3) Fail% function: empty/missing ⇒ 100% (Bad)
        def fail_pct(df):
        ...
        # 4) Verdict: require valid comparisons (no silent pass on empty)
        try:
        ...
        is_good = (v == "Good") and both_present
        ...
        self.card.setText(
            ...
            f"{v} | sim(hist={corr:.2f}, kp={good_kp})"
        )

        # 5) Optional: DB save (kept as non-fatal)
        try:
        ...
        # 7) Per-run comparison export (Thread/Ring compare sheets)
